...
def conv_backward(dZ, cache):
    ...
    for h in range(n_H):  # loop over vertical axis of the output volume
        for w in range(n_W):  # loop over horizontal axis of the output volume
            ...
            dA_prev_pad[:, vert_start:vert_end, horiz_start:horiz_end, :] += np.transpose(np.dot(W, dZ[:, h, w, :].T),
                                                                                          (3, 0, 1, 2))
...
def Conv3D(image3D, filter3D, b, stride):
    ...
    for c in range(n_C):
        ...
        for c_prev in range(n_C_prev):
            output_c += convolve2d(image3D[:, :, c_prev], np.rot90(filter3D[:, :, c_prev, c], 2), 'valid')[::stride,
                        ::stride]
        output_c += b[0, 0, 0, c]
        output += [output_c]
